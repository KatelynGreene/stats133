...
GetEduData <- function(userName){
  ...
  if(userName == "K"){
    ...
    path <- "C:\\Users\\Katrlyn\\stats133Project\\AllStates\\"
  }
...
GetStateRegion <- function(all_data){
  all_data$Region <- ""
  for (i in 1:nrow(all_data)){
    if (all_data$StateFips[i] %in% c(53, 16, 32, 06, 41)){
      all_data$Region[i]<- "Pacific"
    } else if (all_data$StateFips[i] %in% c(35, 04, 49, 40 )){
...
View(all_data)
#all_data <- GetStateRegion(all_data)
#<<<<<<< Updated upstream
