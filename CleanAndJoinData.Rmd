...
all_data_narrow<-all_data %>% 
  gather(key=EducationLevel, value=PercentOfAdults, LessThanHS:Bachelors) %>%
  mutate(bach_HS = Bachelors/HSDiploma) %>%
  mutate(coll_HS = (SomeCollege + Bachelors)/(LessThanHS + HSDiploma))


head(all_data)
head(all_data_narrow)
```
...
income_edu_plot<-all_data_test %>%
  ggplot(aes(coll_HS, Income)) +
  geom_point() +
  geom_smooth(method = "lm")

lm(coll_HS~Pollution  + Income, data=all_data_test) %>% summary()
