...
edu_data<-edu_data_unclean[, -grep("1970|1980|1990", colnames(edu_data))]
edu_data<-edu_data%>%subset(select=-c( `State`, `Area name`))%>%dplyr::rename(FIPSCode=`FIPS Code`)
```
...
poll_county_clean<-read.csv("https://data.cdc.gov/api/views/cjae-szjv/rows.csv?accessType=DOWNLOAD")
head(poll_county)
...
fips_ref<-dplyr::rename(fips_ref, FIPSCode=CountyFIPS)%>%subset(select=-c(ClassFIPSCode))
View(fips_ref)
```
...
income_unclean <- read_excel("C:\\Users\\Katrlyn\\Downloads\\income.xls", sheet = 1, skip = 7)
head(data)
income<- income_unclean %>% mutate(Area_name= gsub(", ..$", "", Area_name), Area_name)
...
```{r}
all_data<-fips_ref%>%full_join(edu_data, by ='FIPSCode')
