...
DownloadLocally <- function(AllStates){
  ...
  for(i in 1:length(AllStates$stateNames)){
    destination <- paste("/Users/anishakumar/Documents/Stats133/RStudioFiles/Stats133Project/AllStates/", AllStates$stateNames[i], ".xls", sep = "")
...
all_data<-plyr::join_all(list(every_county, edu_data, income_data, poll_data, tree_data), by='FIPSCode', type='full')%>%
  mutate(coll_HS = (SomeCollege + Bachelors)/(LessThanHS + HSDiploma))
...
edu_poll_graph<-all_data_narrow%>%
  ggplot(aes(x=PercentOfAdults, y=Pollution))+
  geom_point(aes(color=EducationLevel))+
  facet_grid(EducationLevel~.)
...
income_edu_plot<-all_data %>%
  ggplot(aes(coll_HS, Income)) +
  geom_point() +
  geom_smooth(method = "lm")

lm(coll_HS~Pollution  + Income, data=all_data) %>% summary()

```



Convert income into its quartiles
```{r}
#all_data$quartile <- ntile(all_data$Income, 4)  
View(all_data)
pollutionVRatioQuartiles<-all_data%>%
  filter(!is.na(Income)) %>%
  ggplot(aes(x=Pollution, y=coll_HS))+
  geom_point(aes(color=IncomeQuartile))+
  facet_grid(IncomeQuartile~.)
#View(all_data_narrow)
pollutionVRatioQuartiles 
#education is ratio of higher education to high school or less 
