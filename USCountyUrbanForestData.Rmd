...
#only goes into one state right now
for (i in 1:1){# length(stateLinks)){
  stateURL<- stateLinks[1]
  stateTxt <- getURLContent(stateURL)
  stateDoc <- htmlParse(stateTxt)
  stateExcel <- xpathSApply(stateDoc, '//ol[@id="data_options"]/li/a/@href')
  stateExcel <- as.character(stateExcel)
  length(stateExcel)
  ...
  print(stateExcel) #stateExcel isn't being stored anywayre
}

#stateExcel is only one state URL 
download.file(url=stateExcel,
              destfile='/Users/anishakumar/Desktop', method  = "curl")
