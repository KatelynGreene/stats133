...
all_data <- arsenic_df %>%
  left_join(income, by = c("fips" = "FIPSCode")) %>%
  left_join(edu_data, by = c("fips" = "FIPSCode"))


all_data_test <- arsenic_df %>%
  left_join(income, by = c("fips" = "FIPSCode")) %>%
  left_join(edu_data, by = c("fips" = "FIPSCode"))
```

## Testing different manipulations of variables
```{r}
all_data_test <- all_data_test %>%
  mutate(bach_HS = Bachelors/HSDiploma) %>%
  mutate(coll_HS = (SomeCollege + Bachelors)/(LessThanHS + HSDiploma))
all_data_test$arsenic[all_data_test$arsenic > 100] <- NA


all_data_test %>%
  ggplot(aes(arsenic, bach_HS)) +
  geom_point() +
  geom_smooth(method = "lm")

all_data_test %>%
  ggplot(aes(coll_HS, Median_Household_Income_2015)) +
  geom_point() +
  geom_smooth(method = "lm")

```

## Visualizing arsenic data

```{r}
all_data_test %>%
  ggplot(aes(log(arsenic), log(bach_HS))) +
  geom_point() +
  geom_smooth(method = "lm")
