...
et2<-lm(coll_HS~TreeCanopyCover + UrbanRank, data=all_data)
et3<-lm(coll_HS~TreeCanopyCover + UrbanRank +Income, data=all_data)
...
ep2<-lm(coll_HS~Pollution + UrbanRank, data=all_data)
ep3<-lm(coll_HS~Pollution + UrbanRank + Income, data=all_data) 
...
bach_vs_green_lm <- all_data %>% 
  ...
  geom_smooth(method = "lm") +
  facet_grid(. ~ UrbanRank) +
  labs(x="AvailGreenSpace", y="% Adults with Bachelor's Degrees") +
  theme_bw() 
