...
Pacific_bach_vs_pollution_lm <- all_data %>%
  na.omit() %>%
...
NE_bach_vs_pollution_lm <- all_data %>%
  na.omit() %>%
...
NE_bach_vs_pollution_lm

```
```{r}
Pacific_bach_vs_tree_lm <- all_data %>%
  na.omit() %>%
  filter(Region == "Pacific") %>%
  ggplot(aes(TreeCanopyCover, Bachelors)) +
  geom_point(aes(color = UrbanRank), alpha = 0.3, size = 2) +
...
Pacific_bach_vs_tree_lm

SW_bach_vs_tree_lm <- all_data %>%
  na.omit() %>%
  filter(Region == "SouthWest") %>%
  ggplot(aes(TreeCanopyCover, Bachelors)) +
  geom_point(aes(color = UrbanRank), alpha = 0.3, size = 2) +
